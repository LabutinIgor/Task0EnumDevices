...
//        -30 CL_INVALID_VALUE
        ...
        for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            ...
            size_t memorySize = 0;
            OCL_SAFE_CALL(clGetDeviceInfo(devices[deviceIndex], CL_DEVICE_GLOBAL_MEM_SIZE, 0, nullptr, &memorySize));
            cl_ulong memory;
            OCL_SAFE_CALL(clGetDeviceInfo(devices[deviceIndex], CL_DEVICE_GLOBAL_MEM_SIZE, memorySize, &memory,
                                          nullptr));
            std::cout << "    Device memory: " << memory / (1024 * 1024) << " Mbyte" << std::endl;


            size_t localMemorySize = 0;
            OCL_SAFE_CALL(
                    clGetDeviceInfo(devices[deviceIndex], CL_DEVICE_LOCAL_MEM_SIZE, 0, nullptr, &localMemorySize));
            cl_ulong localMemory;
            OCL_SAFE_CALL(clGetDeviceInfo(devices[deviceIndex], CL_DEVICE_LOCAL_MEM_SIZE, localMemorySize, &localMemory,
                                          nullptr));
            std::cout << "    Device local memory: " << localMemory << " byte" << std::endl;


            size_t extensionsSize = 0;
            OCL_SAFE_CALL(
                    clGetDeviceInfo(devices[deviceIndex], CL_DEVICE_EXTENSIONS, 0, nullptr, &extensionsSize));
            std::vector<char> extensions(extensionsSize);
            OCL_SAFE_CALL(clGetDeviceInfo(devices[deviceIndex], CL_DEVICE_EXTENSIONS, extensionsSize, extensions.data(),
                                          nullptr));
            std::cout << "    Device extensions: " << extensions.data() << std::endl << std::endl;
